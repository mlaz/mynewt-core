...
struct adc_chan_config nrf52_adc_chans[SAADC_CH_NUM];
...
nrf52_adc_sample(struct adc_dev *dev)
{
    ...
    if (g_drv_instance.primary_size <
        (used_chans * sizeof(nrf_saadc_value_t))) {
            return (OS_ENOMEM);
        }
...
static int
nrf52_adc_read_buffer(struct adc_dev *dev,
                      void *buf,
                      int buf_len,
                      int off,
                      int *result)
...
void nrf52_saadc_irq_handler(void)
{
    ...
    if (nrf_saadc_event_check(NRF_SAADC, NRF_SAADC_EVENT_END)) {
        ...
        ev = ADC_EVENT_RESULT;
        global_adc_dev->ad_event_handler_func(
            global_adc_dev,
